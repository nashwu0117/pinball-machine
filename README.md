...
B.B.MAN／彈珠超人類型的 3D 夜市積分彈珠台。玩家選擇基礎分數與本局
彈珠數量，讓鋼珠穿過釘陣，落入 ×1／×2／×3／×5／×10 倍率洞，
依「基礎分數 × 洞口倍率」累積街機分數**（此分數制為瀏覽器遊戲設計抽象，實機無分數概念，贏得的是彈珠顆數）**。

## v7 實機重建版

- 依實機 64 × 35.5 × 47.5 cm 的窄長比例重建桌上型黃色機殼
- 後方直立控制箱包含投幣器、LED 分數顯示、倍率燈與黃色退珠盒
- 球盤底部是 9 個黃色倍率洞：1／2／3／5／10／5／3／2／1
- 前控制板具白色跳燈鈕、中央鎖孔、紅色接珠盤與右前鍍鉻彈簧拉桿
- 彩虹外框、透明罩、卡通印刷盤面與夜市共用鐵架依實機照片整理
- 完整新手引導、中文即時狀態、基礎分數選擇與 5／10／20 顆批量模式
- 總分、最高單次得分、各倍率次數與最近進洞歷史紀錄**（「總分」與「最高單次得分」為瀏覽器遊戲設計抽象，實機無分數概念，贏得的是彈珠顆數）**。
...
Headless 瀏覽器端到端測試（功能 + 物理 + 混沌測試，27 項檢查）：
